package Hypernova::Dimensions::Controller::ApiKeys;
...
sub list {
    ...
    return try {
        ...
        my $objects = $c->schema->resultset('ApiKey')->search($params);
...
sub get {
    ...
    return try {
        my $id = $c->param('id');
        my $object = $c->schema->resultset('ApiKey')->find($id);
...
sub add {
    ...
    return try {
        ...
        my $object = $c->schema->resultset('ApiKey')->create($params);
           $object = $c->schema->resultset('ApiKey')->find($object->id);
