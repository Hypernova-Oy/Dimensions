package Hypernova::Dimensions::Controller::EventLog;
...
sub list {
    ...
    return try {
        ...
        my $objects = $c->schema->resultset('EventLog')->search($params);
...
sub get {
    ...
    return try {
        my $id = $c->param('id');
        my $object = $c->schema->resultset('EventLog')->find($id);
...
sub add {
    ...
    return try {
        ...
        my $object = $c->schema->resultset('EventLog')->create($params);
           $object = $c->schema->resultset('EventLog')->find($object->id);
