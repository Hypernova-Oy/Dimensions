package Hypernova::Dimensions::Controller::Accounts;
...
sub list {
    ...
    return try {
        ...
        my $objects = $c->schema->resultset('Account')->search($params);
...
sub get {
    ...
    return try {
        my $id = $c->param('id');
        my $object = $c->schema->resultset('Account')->find($id);
...
sub add {
    ...
    return try {
        ...
        my $object = $c->schema->resultset('Account')->create($params);
           $object = $c->schema->resultset('Account')->find($object->id);
